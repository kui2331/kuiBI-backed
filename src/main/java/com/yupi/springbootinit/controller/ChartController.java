...
import com.yupi.springbootinit.bizmq.BiMqMessageProducer;
...
//                "{明确的数据分析结论、越详细越好，不要生成多余的注释}";
    ...
    public BaseResponse<BiResponse> genChartByAiAsync(@RequestPart("file") MultipartFile multipartFile, GenChartByAiRequest genChartByAiRequest, HttpServletRequest request)
    {
        ...
        CompletableFuture.runAsync(() -> {
            ...
            String genChart = splits[1].trim();//删除字符串前后的空格
            String genResult = splits[2].trim();

            chart.setGenChart(genChart);
            chart.setGenResult(genResult);
            chart.setStatus("succeed");
            boolean saveResult2 = chartService.updateById(chart);
            ThrowUtils.throwIf(!saveResult2, ErrorCode.SYSTEM_ERROR, "图表异步保存失败");
        });
        BiResponse biResponse = new BiResponse();
//        biResponse.setGenChart(genChart);
//        biResponse.setGenResult(genResult);
